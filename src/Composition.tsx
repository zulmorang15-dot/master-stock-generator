...
const Displex: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Absolutely seamless 15-second loop configuration
    // Symmetrical time progression mapping ensures first and last frames match perfectly
    const halfCycle = durationInFrames / 2;
    const progress = Math.abs((frame - halfCycle) / halfCycle); // 1.0 -> 0.0 -> 1.0
    const maxTime = 18.0; 
    const uTimeVal = (1.0 - progress) * maxTime;

    materialRef.current.uniforms.uTime.value = uTimeVal;
