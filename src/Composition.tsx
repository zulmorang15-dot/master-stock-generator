...
const ElegantLiquidGradient: React.FC = () => {
  ...
  useEffect(() => {
    if (!rendererRef.current || !uniformsRef.current || !sceneRef.current || !cameraRef.current) {
      return;
    }

    const LOOP = 12.0;
    const simulatedTime = (frame / fps) % LOOP;
    uniformsRef.current.uTime.value = simulatedTime;
