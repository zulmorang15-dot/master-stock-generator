...
// JALUR 1: DATA SCRAPING + ATM OPTIMIZATION + KODE DARI NOL
app.post("/api/generate", async (req, res) => {
  const { keyword } = req.body;
  const dataScrap = await scrapAdobeStock(keyword);

  const prompt = `
    Kamu adalah pakar Creative Director SEO Microstock USA dan Senior Animation Developer React/Remotion.
    ...
    Struktur objek wajib persis seperti ini:
    {
      ...
      "promptCode": "Tulis satu file kode React Remotion (.tsx) UTUH untuk komponen 'MyComposition'. WAJIB memenuhi syarat: 1. Jika tema abstrak/techno, buat sistem looping sempurna (seamless loop) memanfaatkan frame/fps agar frame awal dan akhir menyatu. 2. JANGAN beri warna background solid pada tag container terluar, biarkan background-nya TRANSPARAN (tanpa backgroundColor) agar bisa digunakan sebagai overlay video. 3. Buat ornamen visual berlapis (kombinasi fungsi Math.sin/Math.cos untuk pergerakan partikel acak, transformasi CSS 3D, atau efek wave). Teks utama harus memiliki animasi masuk dan keluar yang halus. Langsung keluarkan teks kodenya murni dari baris import sampai export murni tanpa markdown \`\`\`tsx."
    }
  `;
  ...
  try {
    ...
    res.json(dataObjek);
    console.log("✅ Sukses mengirimkan data dan skrip kode bebas AI ke dashboard!");
  } catch (error) {
...
app.post("/api/render", async (req, res) => {
  ...
  try {
    // 1. Timpa file src/Composition.tsx lokal dengan hasil editan Anda di tabel
    fs.writeFileSync("src/Composition.tsx", item.promptCode);
    console.log(`📝 File src/Composition.tsx berhasil diperbarui!`);

    // 2. Jalankan Auto Push ke GitHub secara sinkronus
    ...
    // 3. Tembak API GitHub Actions untuk Cloud Rendering (.mov ProRes Alpha)
